...
Select title, length, rank() over (order by length) as my_rank from sakila.film
...
Select title, length, rating, rank() over (partition by rating order by length) as my_rank from sakila.film
where rating not in('',0,' ')
...
SELECT r.name, l.category_id, count(a.film_id) as num_films from sakila.film_category as l
join sakila.film as a
on l.film_id=a.film_id
join sakila.category as r
on l.category_id=r.category_id
group by l.category_id;

#4. Which actor has appeared in the most films?
SELECT a.first_name, a.last_name, count(l.film_id) as num_films from sakila.film_actor as l
...
Select a.customer_id, a.first_name, a.last_name, count(rental_id) as num_rentals from sakila.rental as l
